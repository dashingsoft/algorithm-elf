...
注册本产品之后，可以永久使用本产品并且在整个产品周期之后可以
免费升级。
...
1. 申请注册码。
   ...
   或者启动主程序，点击菜单 帮助 -> 在线注册。

   这样会自动弹出注册网页，按照页面提示进行操作即可购买注册码。
...
2. 生成认证文件。
   ...
   获得注册码之后，启动算法精灵，点击菜单 帮助->生成认证文件，
   打开注册窗口。

   a. 将邮件中包含注册码拷贝到下面的编辑框中。

   b. 点击按钮【校验】，完成产品注册。

   注册成功之后主窗口的注册按钮不再显示，如果你需要重新注册，
   可以删除安装目录下的 license.lic 即可。
...
algorithm-elf-X.Y.M.N-1.linux.i686.rpm

2.2 安装

安装算法精灵使用命令：

$ rpm -ivh algorithm-elf-X.Y.M.N-1.linux.i686.rpm
